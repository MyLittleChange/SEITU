...
from data.vcr_comet import Comet_qa_match_VcrDataset,Comet_qa_match_vcr_collate,Comet_qa_match_val_VcrDataset,Comet_qa_match_val_vcr_collate
...
def main(opts):
    ...
    train_dataset=Comet_qa_match_VcrDataset(tokenizer,qa_txt_db, img_db_gt=img_db_gt, img_db=img_db)
    train_dataloader = build_dataloader(train_dataset, Comet_qa_match_vcr_collate, True, opts)
    ...
    val_dataset = Comet_qa_match_val_VcrDataset(tokenizer,val_txt_db, img_db=val_img_db, img_db_gt=val_img_db_gt)
    val_dataloader = build_dataloader(val_dataset, Comet_qa_match_val_vcr_collate,
                                      False, opts)
    ...
    if opts.global_step==0:
        ...
        # model_encoder.load_state_dict(matched_state_dict, strict=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_dir", default='./outputs/qa_comet_match', type=str,
    ...
    if args.do_predict:
        ...
        tokenizer = BertTokenizer.from_pretrained('bert-base-cased', do_lower_case=False)
        val_final_dataset = Comet_qa_match_val_VcrDataset(tokenizer,val_txt_db, img_db=val_img_db, img_db_gt=val_img_db_gt)
        val_final_dataloader = build_dataloader(
            val_final_dataset, Comet_qa_match_val_vcr_collate,
            False, args)
