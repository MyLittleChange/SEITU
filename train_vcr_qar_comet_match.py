...
from model.vcr_caption import UniterForVisualCommonsenseReasoning_match
from data.vcr_comet import Comet_qar_match_VcrDataset,Comet_qar_match_vcr_collate,Comet_qar_match_val_VcrDataset,Comet_qar_match_val_vcr_collate
...
def main(opts):
    ...
    train_dataset=Comet_qar_match_VcrDataset(tokenizer,qa_txt_db, img_db_gt=img_db_gt, img_db=img_db)
    train_dataloader = build_dataloader(train_dataset, Comet_qar_match_vcr_collate, True, opts)
    ...
    val_dataset = Comet_qar_match_val_VcrDataset(tokenizer,val_txt_db, img_db=val_img_db, img_db_gt=val_img_db_gt)
    val_dataloader = build_dataloader(val_dataset, Comet_qar_match_val_vcr_collate,
                                      False, opts)
    ...
    model_encoder = UniterForVisualCommonsenseReasoning_match.from_pretrained(opts.encoder_config, checkpoint, img_dim=IMG_DIM)
    ...
    while n_epoch<opts.epoch:
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            rank_scores,cls_loss,match_loss,matched = model_encoder(batch)
            match_loss=match_loss * 0.5
            ...
            tr_match_loss += match_loss.item()
            total_step += 1
            pbar(step=step, info={'loss': tr_loss / total_step,'tr_cls_loss': tr_cls_loss / total_step,'tr_match_loss': tr_match_loss / total_step,'a_acc':n_correct_qa/n_examples})

            if global_step%20000==0:
                ckpt_dir=os.path.join(opts.output_dir,'ckpt')
                output_dir = os.path.join(ckpt_dir,
                                          "UNITER-GPT-checkpoint-{}-.tar".format(global_step))
                torch.save({
                    'epoch': n_epoch,
            ...
            if global_step % opts.valid_steps == 0 and n_epoch>=opts.epoch_begin:
                ...
                ckpt_dir = os.path.join(opts.output_dir, 'ckpt')
                output_dir = os.path.join(ckpt_dir,
                                          "UNITER-GPT-checkpoint-{}-{:4f}.tar".format(global_step,score))
                torch.save({
                    'epoch': n_epoch,
                    'model_state_dict': model_encoder.state_dict(),
                    'optimizer_state_dict': optimizer.state_dict(),
...
def validate(model, val_loader,args):
    ...
    for i, batch in enumerate(val_loader):
        ...
        with torch.no_grad():
            rank_scores,loss,_,matched_qa= model(batch)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_dir", default='./outputs/qar_comet_match', type=str,
    ...
    # training parameters
    parser.add_argument("--num_sample_batch", default=2, type=int,
    ...
    if args.do_predict:
        n_gpu = args.num_gpu
        checkpoint={}
        model_encoder = UniterForVisualCommonsenseReasoning_match.from_pretrained(
        ...
        tokenizer = BertTokenizer.from_pretrained('bert-base-cased', do_lower_case=False)
        val_final_dataset = Comet_qar_match_val_VcrDataset(tokenizer,val_txt_db, img_db=val_img_db, img_db_gt=val_img_db_gt)
        val_final_dataloader = build_dataloader(
            val_final_dataset, Comet_qar_match_val_vcr_collate,
            False, args)
