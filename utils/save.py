...
class ModelSaver(object):
    ...
    def save(self, model, step, epoch,optimizer=None):
    ...
    def save_score(self, model, step,score, epoch,optimizer=None):
        output_model_file = join(self.output_dir,
                                 f"{self.prefix}_{step}_{round(score,3)}.{self.suffix}")
        state_dict = {k: v.cpu() if isinstance(v, torch.Tensor) else v
                      for k, v in model.state_dict().items()}
        state_dict['epoch'] = epoch
        torch.save(state_dict, output_model_file)
        if optimizer is not None:
            ...
            if hasattr(optimizer, '_amp_stash'):
                pass  # TODO fp16 optimizer
            torch.save(dump, f'{self.output_dir}/train_state_{step}_{round(score,3)}.pt')