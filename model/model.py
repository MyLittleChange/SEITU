...
class UniterModel_add(UniterPreTrainedModel):
    """ Modification for Joint Vision-Language Encoding
    """
    def __init__(self, config, img_dim):
        super().__init__(config)
    ...
    def _compute_img_txt_embeddings(self, input_ids, position_ids,
                                    img_feat, img_pos_feat,
                                    gather_index, img_masks=None,
                                    txt_type_ids=None, img_type_ids=None,input_region=None):
        img_emb = self._compute_img_embeddings(
            img_feat, img_pos_feat, img_masks, img_type_ids)
        txt_emb = self._compute_txt_embeddings(
            input_ids, position_ids, txt_type_ids)
        for index in range(len(input_region)):
            region=input_region[index]
            for i in range(len(region)):
                if region[i]!=0:
                    #加上对应位置的图片向量
                    #注意需要-1，因为起始位置不同
                    txt_emb[index][i]+=img_emb[index][region[i]-1]
    ...
    def forward(self, input_ids, position_ids,
        ...
        if input_ids is None:
            ...
            embedding_output = self._compute_img_txt_embeddings(
                input_ids, position_ids,
                img_feat, img_pos_feat,
                gather_index, img_masks, txt_type_ids, img_type_ids,input_region)
...
class UniterModel_graph(UniterPreTrainedModel):
    """ Modification for Joint Vision-Language Encoding
    """
    def __init__(self, config, img_dim):
        super().__init__(config)
    ...
    def _compute_img_txt_embeddings(self, input_ids, position_ids,
                                    img_feat, img_pos_feat,
                                    gather_index, img_masks=None,
                                    txt_type_ids=None, img_type_ids=None,input_region=None,vis_edge=None):
        img_emb = self._compute_img_embeddings(
            img_feat, img_pos_feat, img_masks, img_type_ids)
        txt_emb = self._compute_txt_embeddings(
            input_ids, position_ids, txt_type_ids)
        txt_emb_new=torch.zeros(txt_emb.size(),dtype=txt_emb.dtype).cuda()
        txt_emb_new[:,:,:]=txt_emb[:,:,:]
        for index in range(len(input_region)):
            region=input_region[index]
            for i in range(len(region)):
                if region[i]!=0:
                    #加上对应位置的图片向量
                    #注意需要-1，因为起始位置不同
                    alpha=torch.cat((txt_emb[index][i],img_emb[index][region[i]-1]),dim=-1)
                    alpha=self.linear_1(alpha)
                    alpha=self.sig(alpha)
                    txt_emb_new[index][i]=(1-alpha).mul(txt_emb[index][i])+alpha.mul(img_emb[index][region[i]-1])

        img_emb_new=torch.zeros(img_emb.size(),dtype=img_emb.dtype).cuda()
        img_emb_new[:,:,:]=img_emb[:,:,:]
        for index in range(len(vis_edge)):
            edge=vis_edge[index]
            for i in range(len(edge[0])):
                for j in range(len(edge[1])):
                    if edge[i][j]!=0:
                        beta=torch.cat((img_emb[index][i],img_emb[index][j]),dim=-1)
                        beta_=self.linear_1(self.sig(beta))
                        img_emb_new[index][i]=(1-beta_).mul(img_emb[index][i])+beta_.mul(img_emb[index][j])

        # align back to most compact input
        gather_index = gather_index.unsqueeze(-1).expand(
            -1, -1, self.config.hidden_size)
        embedding_output = torch.gather(torch.cat([txt_emb_new, img_emb_new], dim=1),
                                        dim=1, index=gather_index)
        return embedding_output
    ...
    def forward(self, input_ids, position_ids,
        ...
        if input_ids is None:
            ...
            embedding_output = self._compute_img_txt_embeddings(
                input_ids, position_ids,
                img_feat, img_pos_feat,
                gather_index, img_masks, txt_type_ids, img_type_ids,input_region,vis_edge)
...
class GRU_encoder(UniterPreTrainedModel):
    """ Modification for Joint Vision-Language Encoding
    """
    def __init__(self, config, img_dim):
        super().__init__(config)
        self.embeddings = UniterTextEmbeddings(config)
        self.img_embeddings = UniterImageEmbeddings(config, img_dim)
        self.encoder=nn.GRU(input_size=config.hidden_size,hidden_size=512,num_layers=1)
        self.apply(self.init_weights)
    ...
    def forward(self, input_ids, position_ids,
        ...
        encoder_output,encoder_hidd = self.encoder(embedding_output)
        return encoder_output,encoder_hidd